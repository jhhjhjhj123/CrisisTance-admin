...
auth.onAuthStateChanged((user) => {
  if (!user) {
    ...
    const soloParentBody = document.getElementById("solo-parent-tbody");

    document
      .getElementById("btn-solo-parent")
      .addEventListener("click", displayStaffs);

    displayStaffs();

    async function displayStaffs() {
      soloParentBody.innerHTML = "";
      ...
      for (const ID in snapshotData) {
        ...
        if (data.typeOfAssistance === "Solo Parent" && data.approved === true) {
